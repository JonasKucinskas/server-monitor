...
public class Process
{
    ...
    public async Task InsertToDatabase(NpgsqlConnection conn)
    {
        await using var cmd = new NpgsqlCommand(@"
            INSERT INTO processes_data (pid, process_user, process_time, system_ip, name, cpu_usage, ram_usage, notification_rule_id)
            VALUES (@pid, @process_user, @process_time, @system_ip, @name, @cpu_usage, @ram_usage, @notification_rule_id);
        ", conn);
